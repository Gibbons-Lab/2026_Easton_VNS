...
params.data_dir = "/proj/gibbons/2024_easton_vns/metaphlan_2"
params.fastq_dir="/proj/gibbons/2024_easton_vns/data/fastq/raw"
...
process preprocess {
    cpus 4
    publishDir "${params.data_dir}/metaphlan_preprocessed"
...
process metaphlan {
    ...
    if (params.single_end)
        """
        metaphlan ${params.data_dir}/metaphlan_preprocessed/${reads[0]} --input_type fastq --db_dir ${params.mpa_db} --mapout ${id}_metagenome.bowtie2.bz2 --nproc ${task.cpus} -o ${id}_metaphlan.txt -x ${params.mpa_index} -t rel_ab_w_read_stats
        """
    else
        """
        metaphlan ${params.data_dir}/metaphlan_preprocessed/${reads[0]},${params.data_dir}/metaphlan_preprocessed/${reads[1]} --input_type fastq --db_dir ${params.mpa_db} --mapout ${id}_metagenome.bowtie2.bz2 --nproc ${task.cpus} -o ${id}_metaphlan.txt -x ${params.mpa_index} -t rel_ab_w_read_stats
        """
}
...
process multiqc {
    ...
    """
    multiqc ${params.data_dir}/metaphlan_preprocessed ${params.data_dir}/metaphlan
    """
}
